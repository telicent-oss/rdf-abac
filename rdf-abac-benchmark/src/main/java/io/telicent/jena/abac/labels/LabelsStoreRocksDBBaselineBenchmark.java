 ...
 * across time. - read_hot_hits:   100% hits on existing triples - read_hot_mixed:  50% hits, 50% misses They: - Build a
 * RocksDB label store in a temp directory - Pre-populate it with a configurable number of triples - Pre-generate the
 * read workloads so the benchmark body does minimal work It's a starting point and an approach we can apply to all
 * appropriate repos.
 */
...
public class LabelsStoreRocksDBBaselineBenchmark {
    ...
    /**
     * Number of SPO triples to load into the label store. This determines both DB size and the size of the "hot" read
     * set.
     */
    ...
    public void setUp() throws IOException {
        ...
        labelsStore = new LegacyLabelsStoreRocksDB(
                helper,
                dbDir,
                storeFmt,
                null
    ...
    /**
     * Generate a reproducible SPO triple for index {@code i}. The pattern ensures: - many distinct objects - repeated
     * subjects/predicates (more realistic index behaviour)
     */
    ...
    private Label generateRandomLabels() {
        ...
        return Label.fromText(RandomStringUtils.insecure()
                                               .nextAlphanumeric(multiplier * LABEL_TEXT_LENGTH));
    }
}
